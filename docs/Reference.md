...
- 大容量：支持亿级以上的键值对。
- 时间复杂度：最好的情况为O(m)，最坏的情况为O(mlogn)，m 为字符串长度，n 为256。具体[扩容与缩容](##### 8.1.2.4.扩容缩容)解释见 <a href="#time">为什么时间复杂度最坏的情况下为 O(mlogn)？</a>。
...
总而言之，开源框架面对的是通用场景，而不是某个特定场景，因此需要考虑更多情况，以达到某种意义上的平衡。
...
Java使用的 UTF-16 字符集的字符数为65536。当容量为128时，单一位置最多会有 65536 ÷ 128 = 512 个字符节点。因为扩容因子为 2，所以 size 达到256 个字符的时候 table 会扩容到256，这时单一位置最多就只有 65536 ÷ 256 = 256 个字符，因此 n 最大为256。

一旦单一位置的节点数达到阈值8，LinkedNode 就会转换为 AvlNode。AVLTree 的时间复杂度为O(logn)，再乘于字符串的长度 m，查找整个字符串 key 的时间复杂度就是 O(mlogn)。
